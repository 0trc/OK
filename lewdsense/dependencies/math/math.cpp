...
void sin_cosb(float r, float* s, float* c) {
	*s = sin(r);
	*c = cos(r);
}

void angle_vectorsb(vec3_t& angles, vec3_t& forward) {
	float sp, sy, cp, cy;



	sin_cosb(DEG2RAD(angles.x), &sp, &cp);
	sin_cosb(DEG2RAD(angles.y), &sy, &cy);
...
float math::distance_based_fov(float distance, vec3_t angle, c_usercmd* cmd) {
	vec3_t aiming_at;
	angle_vectorsb(cmd->viewangles, aiming_at);
	aiming_at *= distance / 10;

	vec3_t aim_at;
	angle_vectorsb(angle, aim_at);
	aim_at *= distance / 10;
...
vec3_t math::calculate_angle_alternative(vec3_t& a, vec3_t& b) {
	vec3_t angles;
